...
% Existem muitas implementacoes diferentes da linguagem: client-side (browsers), server-side (Node.js), embedded devices (Jerryscript) -> Estas implementacoes têm de estar de coincider no comportamento observavel -> é particularmente importante na Web -> senao temos sites que em ...
JavaScript (JS) is a programming language mainly used in the development of client side web applications, also being one of the most popular programming languages. According to both GitHub and StakeOverflow statistics, JavaScript finished 2021 as \colorbox{orange}{second most active} languages on GitHub\footnote{Second most utilized language based GitHub pull requests - https://madnight.github.io/githut/} and the second most active on StackOverflow.\footnote{Tendencies based on the Tags used - https://insights.stackoverflow.com/trends}
...
\paragraph{Example of Array.pop}
Figure \ref{fig:Array_pop_example} shows a simplified version of before \texttt{(a)} and after \texttt{(b)} of an array performing the pop function.
Before preforming \texttt{pop} \texttt{(a)}, the array has three  properties \texttt{length}, \texttt{0}, and \texttt{1}. Property \texttt{length} represents the size of the array that has value \texttt{2}. While the properties \texttt{0} and \texttt{1} store the first and second elements of the array respectively.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=1\textwidth]{images/test262_array_test.png}
