...
JavaScript is defined in the \texttt{ECMAScript} standard, a document written in English. The standard has be growing with each version, both in size and complexity. The growth of the standard is illustrated in Figure \ref{fig:paginas_standard_version}. This makes the JavaScript a language very hard to implement with many corner cases. \texttt{Test262} was created to aid the development of JavaScript engines, by providing a conformance test suite that developer can use to test their implementations. As the standard is extensive and complex and its conformance test suite needs to thoroughly test the standard implementation, then, the test suite it self becomes very extensive. The standard being complex and extensive also leads to many of its implementations being only partial.
...
When implementing a JavaScript engine it is very important to know the tests that the engine should pass for the feature that is being implemented. The selection of applicable tests for partial implementations is particularly difficult since tests for one feature may or may not use other features that are not implemented. \texttt{MetaData262 v0} appears to solve this issue, it processes the \texttt{Test262} suite and calculates metadata about the tests. \texttt{MetaData262 v0} has some particularly interesting properties such as a version of the standard that the test belongs to, the syntactic constructs that are used in the test, and the built-ins that are used by the test. The metadata of \texttt{MetaData262 v0} is made available in the form of a MongoDB database.
...
In this thesis we will improve \texttt{MetaData262 v0} in its calculations of exiting metadata as well as adding the new metadata related with the harness functions used. We will also be building web application that interacts with the MongoDB database, allowing users to filter tests by their fields displaying the metadata of the tests along side statistics of the metadata. The aim of the web application is to make the metadata easily accessible.


% Para 6 --> resumo do resto do relatorio
This document is structured in the following way. Section \ref{sec:Goals} has the objectives of this thesis. Section \ref{sec:Background} describes the field in which this thesis is inserted, describing the \texttt{ECMAScript} standard, followed by a description the \texttt{Test262} test suite, and a description of \texttt{MetaData262 v0}. Next is section \ref{sec:Related Work} that presents the scientific work done that relates to this thesis theme. In section \ref{sec:Design and Methodology} is the design and methodology that this thesis is implemented with. As for section \ref{sec:Evaluation and Planning}, it describes how we plan on evaluating the results of this thesis as well as its timeline. Finally, section \ref{sec:Conclusion} is where we present the main conclusions and a summary of this proposal. 
